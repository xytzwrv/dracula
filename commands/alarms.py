...
class AlarmsCog(commands.Cog):
    ...
    def add_reminder(self, user_id: int, reminder_time: datetime, message: str):
        """
    ...
    # Updated command: accepts separate date and time arguments.
    @commands.command(name="setreminder")
    async def set_reminder(self, ctx, date_str: str, time_str: str, *, message: str):
        """
        Sets a reminder.
        Date and time format: YYYY-MM-DD HH:MM (UTC)
        Example: Dracula setreminder 2025-12-31 23:59 "Happy New Year!"
        """
...
async def setup(bot):
    await bot.add_cog(AlarmsCog(bot))
